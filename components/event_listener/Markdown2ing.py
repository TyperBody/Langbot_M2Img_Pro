...
# 预编译正则表达式以提高性能
MARKDOWN_PATTERNS = {
    '标题': re.compile(r'^#{1,6}\s+.+'),
    '粗体': re.compile(r'\*\*.+?\*\*|__.+?__'),
    '斜体': re.compile(r'\*[^*\n].*?\*|_[^_\n].*?_'),
    '行内代码': re.compile(r'`[^`\n]+?`'),
    '列表': re.compile(r'^[\s]*[-*+]\s+.+|^[\s]*\d+\.\s+.+'),
    '引用块': re.compile(r'^>+\s+.+'),
    '链接': re.compile(r'\[.*?\]\(.*?\)'),
    '图片': re.compile(r'!\[.*?\]\(.*?\)'),
    '分割线': re.compile(r'^---+\s*$|^\*\*\*+\s*$'),
    '代码块标记': re.compile(r'^```\s*$|^```\w+')
}
...
def sync_render_markdown(style, markdown_text):
    """
    同步渲染Markdown - 在单独的线程中执行
    """
    try:
        return style.Render(markdown_text)
    except Exception as e:
        print(f"同步渲染Markdown时出错: {e}")
        raise

async def render_markdown_async(style, markdown_text):
    """
    使用ThreadPoolExecutor进行异步渲染，避免事件循环冲突
    """
    try:
        ...
        print(f"异步渲染Markdown时出错: {e}")
        raise

async def safe_render_markdown(style, markdown_text, timeout=8):
    """
    带超时控制的渲染
    """
    try:
        result = await asyncio.wait_for(
            render_markdown_async(style, markdown_text),
            timeout=timeout
        )
        return result
    except asyncio.TimeoutError:
        print(f"Markdown渲染超时（{timeout}秒）")
        return None
    except Exception as e:
        print(f"渲染过程中发生错误: {e}")
        return None

def analyze_line_markdown_optimized(line: str) -> tuple[bool, list[str], bool]:
    """
    优化后的Markdown分析函数
    """
    line = line.strip()
    ...
    for feature_name, pattern in MARKDOWN_PATTERNS.items():
        if pattern.search(line):
            features.append(feature_name)
            
            # 只在链接和图片特征中检查URL
            if feature_name in ['链接', '图片'] and 'http' in line:
                has_url = True
    
    return len(features) > 0, features, has_url

async def process_markdown_analysis_async(lines):
    """
    异步处理Markdown分析
    """
    loop = asyncio.get_event_loop()
    ...
    for line in lines:
        task = loop.run_in_executor(
            None, 
            analyze_line_markdown_optimized, 
            line
        )
    ...
    for result in results:
        if isinstance(result, Exception):
            processed_results.append((False, [], False))
        else:
...
class DefaultEventListener(EventListener):
    ...
    async def initialize(self):
        ...
        async def handler(event_context):
            ...
            global get_url, get_markdown, test_mode, breakout, response_text
            ...
            if test_mode:
                print(f"原始响应文本长度: {len(response_text)}")
            ...
            if len(response_text) > 10000:
                ...
                # 异步分析所有行
                analysis_results = await process_markdown_analysis_async(lines)
                ...
                for i, (is_markdown, features, has_url) in enumerate(analysis_results, 1):
                    if has_url:
                        url_lines += lines[i-1].strip() + "\n"
                        have_url = True
                    ...
                    if test_mode:
                        ...
                        print(f"第{i}行: {status}{features_str} -> {repr(lines[i-1][:50])}")
            ...
            if markdown_number <= 0:
                try:
                    ...
                    # 使用带超时的异步渲染
                    result = await safe_render_markdown(style, response_text, timeout=8)
                    
                    if result and hasattr(result, 'image'):
                        # 异步处理图像转换
                        loop = asyncio.get_event_loop()
                        base64_image = await loop.run_in_executor(
                            None, image_to_base64, result.image
                        )
                        
                        print(f"✅ 转换成功")
                        if test_mode:
                            print(f"Base64图像长度: {len(base64_image)}")
                        
                        # 构建消息链
                        message_chain = []
                        message_chain.append(platform_message.Image(base64=base64_image))
                        
                        if have_url and url_lines.strip():
                            message_chain.append(platform_message.Plain(text=url_lines))
            ...
            if execution_time > 2:
                print(f"⏱️ 插件总执行时间: {execution_time:.2f}秒")